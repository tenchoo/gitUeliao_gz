define(function(require, exports, module) {
  ...
  function updateAddBtn($form, data) {
    $form.find('input[name="form[pieceId]"]').val(data.pieceId);
    $form.parents('li:first').find('[data-templateid]').removeClass('text-disabled').prop('disabled', false).data('parentid', data.pieceId);
    $form.parents('.clearfix:first').find('[data-href]').each(function() {
      var $t = $(this);
      $t.attr('href', $t.data('href').replace('pieceId.', data.pieceId + '.'));
    });
    updateList();
  }

  function createChild($t, pieceId) {
  ...
  $categoryList.on('click', '.name .glyphicon', function(event) {
    ...
    $.post('/content/helpcategory/move', {
      'form[pieceId]': $li.find('input[name="form[pieceId]"]:first').val(),
      'form[to]': 'up'
    }, function(res) {
      if (res.state) {
    ...
    $.post('/content/piece/del', {
      pieceId: $li.find('input[name="form[pieceId]"]:first').val()
    }, function(res) {
      if (res.state) {
        $li.fadeOut(function() {
          $li.remove();
