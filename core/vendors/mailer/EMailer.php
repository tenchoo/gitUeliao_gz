...
class EMailer
{
    ...
    */
	public function __construct($value=null)
	{
		...
		if(empty($value)){
			//获取配置
			$value = Yii::app()->params['mailconfig'];			
			$mailSentType = Yii::app()->params['mailType'];//配置反系列化
			//本地服务器
			if($mailSentType == 'smtp'){
				$this->_myMailer->IsSMTP();
				$this->_myMailer->SMTPAuth = true;
				//配置
			foreach($value as $key=>$val){
				$this->_myMailer->$key = $val;
			}
			}elseif($mailSentType == 'phpmail'){
				$this->_myMailer->IsMail();
			}
   ...
   public function getViewMobil($view, $vars = array(), $layout = null)		//手机验证模板
   {
   	$body = Yii::app()->controller->renderPartial($this->pathViews.'.'.$view, array_merge($vars, array('content'=>$this->_myMailer)), true);
   	if ($layout === null) {
   		$this->_myMailer->Body = $body;
