  ...
  <div class="panel-body">
    <form role="search" class="pull-left form-inline" action="<?php echo $this->createUrl('index');?>">
      ...
      <button class="btn btn-sm btn-default">查找</button>      
        <?php
        if($_GET && $data_list && $this->checkAccess('/statistic/excel/saleman')) {
          echo CHtml::link('导出数据', $this->createUrl('/statistic/excel/saleman', $_GET), ['class'=>"btn btn-primary",'role'=>"button"]);
        }
         ?>
    </form>
  </div>
...
<?php if($data_list):?>
  <div class="text-right"><?php echo CHtml::link('明细报表', $this->createUrl('salemandetail', $_GET));?></div>
<?php endif; ?>
<!-- 数据显示区结束 -->
<script>
