...
//		$this->authenticateToken();
	...
	public function actionCreate() {
		...
		if(is_null($event)) {
			$event = $this->getRequestParams('type');
		}
	 ...
	 * @return bool
	 */
	private function checkParam($field, $validate=null) {
		if(is_null($field)) {
			$this->showJson(false, Yii::t('restful', 'Not found {filed}', ['{filed}'=>$field]));
			return false;
		}
		elseif(!is_null($validate) && !in_array($field, $validate)) {
			$this->showJson(false, Yii::t('restful', 'Invalid {filed} value', ['{filed}'=>$field]));
			return false;
		}
		return true;
	}
