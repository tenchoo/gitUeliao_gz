define(function(require, exports, module) {
  ...
  function updateAddBtn($form, data) {
    $form.find('input[name="form[recommendId]"]').val(data.recommendId);
    $form.parents('li:first').find('[data-templateid]').removeClass('text-disabled').prop('disabled', false).data('parentid', data.recommendId);
    $form.parents('.clearfix:first').find('[data-href]').each(function() {
      var $t = $(this);
      $t.attr('href', $t.data('href').replace('recommendId.', data.recommendId + '.'));
    });
    updateList();
  }

  function createChild($t, recommendId) {
  ...
  $categoryList.on('click', '.name .glyphicon', function(event) {
      ...
      $.post('/content/helpcategory/move', {
        'form[recommendId]': $li.find('input[name="form[recommendId]"]:first').val(),
        'form[to]': 'up'
      }, function(res) {
        if (res.state) {
      ...
      $.get('/content/recommend/createjs', {
        recommendId: $li.find('input[name="form[recommendId]"]:first').val()
      }, function(res) {
        if (res.state) {
    ...
    .on('click', '.cate2 a:not(.del)[href$="javascript::"]', function(e) {
      e.preventDefault();
      alert('推荐位名称不可为空白！');
    })
