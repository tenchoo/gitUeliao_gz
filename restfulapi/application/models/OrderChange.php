...
class OrderChange extends CFormModel {
	...
	*/
	public function change( $dataArr,$model ){
		...
		if(empty($dataArr['products'])){
			$this->addError( 'state','无购买产品信息，若不需购买请直接取消订单!' );
			return false;
		}
	...
	private function updateOrderInfo( $model,$change,$products ){
		...
		foreach ( $model->products as $val){
			...
			$changeInfo = array();
			if( isset($products[$val->orderProductId])  && $products[$val->orderProductId]>0 ){
			...
			if( isset($products['isSample']) && $products['isSample'] == '1' && $val->num < 5 ){
				$val->isSample = 1;
			}else{
				$val->isSample = 0;
			}
		...
		$model->realPayment = $realPayment +  $change->freight;
	...
	*/
	private function notityChange($model,$changeInfo){
		...
		foreach( $distribution->detail as $val ){
			//仓库总分配分拣数量.
			$total[$val->warehouseId] = (isset($total[$val->warehouseId])?$total[$val->warehouseId]:0) + $val->distributionNum;
	...
	*/
	public function check( $dataArr,$applyChange,$model ){
		...
		if( $this->state != '1' && empty( $this->checkInfo )){
	...
	$criteria=new CDbCriteria;
		...
		if( Yii::app()->user->getState('usertype') == tbMember::UTYPE_SALEMAN ){
			...
			if( tbConfig::model()->get( 'default_saleman_id' ) == Yii::app()->user->id ){
				$condition['userId'] = array(0,Yii::app()->user->id);
			}else{
