...
class PackingForm extends CFormModel {
	...
	*/
	 ...
	 */
	public function confirm($positionId,$orderProductId,$unit,$int,$remainder,$singleNumber,&$packDetail){

				  $intPackingNum = bcmul($unit,$int);
					$remainderTotal= array_sum($remainder);
			  //整料和零码的仓库相同时
			  if($positionId[0] == $positionId[1]){
					  	$total = $this->getTotalNum($positionId[0],$singleNumber);//库存总量
					    $lockTotal=$this->getLockTotalNum($positionId[0],$singleNumber);  //锁定库存总量
			  	...
			  	//整料仓库
			  	$total1 = $this->getTotalNum($positionId[0],$singleNumber);//库存总量
					$lockTotal1=$this->getLockTotalNum($positionId[0],$singleNumber);  //锁定库存总量
					...
					//零码仓库
					$total2 = $this->getTotalNum($positionId[1],$singleNumber);//库存总量
					$lockTotal2=$this->getLockTotalNum($positionId[1],$singleNumber);  //锁定库存总量
         ...
         foreach ( $remainder as $key => $value ) {
	       ...
	       //保存分拣员、改变状态
			  if( !$this->packUser( $orderProductId )  ) {
				...
				//写入产品的分拣数量
				if( !$this->product( $orderProductId,$unit,$int,$remainder ) ){
							$transaction->rollback();
							return false;
				}
	...
	public function orderMerge( $orderProductId,$positionId ){
			...
			if(!$merge){
				if($positionId[0] == $positionId[1]){
						...
						$order->warehouseId = tbWarehousePosition::model()->getWarehouseID($positionId[0]);
						if( !$order->save()  ) {
					  ...
					  $order->orderId = $pack->orderId;
						...
						$order->orderId = $pack->orderId;
						$order->warehouseId = tbWarehousePosition::model()->getWarehouseID($positionId[1]);
						if( !$order->save()  ) {
							$this->addErrors( $order->getErrors() );
							return false;
						}
	 ...
	 * @param  int $orderProductId 订单明细ID
	 */
	public function product( $orderProductId ,$unit,$int,$remainder){
        $product = tbOrderProduct::model()->findByPk($orderProductId);
        $intPackingNum = bcmul($unit,$int);
			  $remainderTotal= array_sum($remainder);
			  $num = bcadd($intPackingNum, $remainderTotal);
			  $product->num = $num;
			  $product->remark = $int.'*'.$unit.','.$remainderTotal;
			  if( !$product->save()  ) {
   ...
   */
  public function getTotalNum($positionId,$singleNumber){
    $model =tbWarehouseProduct::model()->findByAttributes(array('positionId'=>$positionId,'singleNumber' =>$singleNumber));
    if( $model ) {
      return $model->num;
    }
    return '';
   ...
   * @param return boolean
   */
  public function getLockTotalNum($positionId,$singleNumber){

    	$model = tbWarehouseLock::model()->findAll(array(
							  'select' =>array('num'),
							  'condition'=>'positionId=:positionId AND singleNumber=:singleNumber',
  							'params' => array(':positionId'=>$positionId,':singleNumber'=> $singleNumber),
							));
    if( $model ) {
