	...
	<ul class="list-group">
	 ...
	 <li class="list-group-item clearfix">
	  <span class="col-md-4">联系人：<?php echo $model->name;?>（<?php echo $model->tel;?>）</span>
		<span class="col-md-8">收货地址：<input type="text" value="<?php echo $model->address;?>" name="data[address]" class="form-control input-sm input-note"/></span>
	 </li>
	 <li class="list-group-item clearfix">
...
<table class="table table-condensed table-bordered order">
    ...
    <tr>
	 ...
	 <td>颜色</td>
	 <td width="10%">购买数量</td>
	 <td width="15%">备货数量<?php if( $model->state == '1' ){ ?><span class="text-danger">(未完成)</span><?php }?></td>
	 <td width="15%">结算数量</td>
	 <td>单价（元）</td>
	 <td>金额（元）</td>
	 <td>赠板</td>
	...
	<td><input type="text" name="data[products][<?php echo $pval->orderProductId;?>][settlementNum]" value="<?php echo $pval['num'];?>" class="form-control num-float-only input-sm" max="<?php echo $pval['num'];?>"/> </td>
	...
	<tr>
		<td colspan="10" align="right">
		 <span>物流费 ： <input type="text" name="data[freight]" value="<?php echo number_format($model->freight,2);?>" class="form-control input-sm price-only"/></span>
		 <span class="p-total">商品金额 ：<?php echo number_format($totalPayments,2);?></span>
		<span class="a-total">总金额 ：<?php echo number_format($totalPayments+$model->freight,2);?></span>
	    </td></tr>
       </tbody>
</table>
