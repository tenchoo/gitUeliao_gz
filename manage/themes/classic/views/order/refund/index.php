<link rel="stylesheet" href="/themes/classic/statics/app/order/css/style.css" />
  ...
  <colgroup><col><col width="12%"><col width="12%"><col width="15%"><col width="15%"><col width="12%"></colgroup>
   ...
   <td><?php echo Order::quantityFormat($pval['num']);?></td>
	...
	<?php if( $val['state'] == '2') { ?>
		<a href="<?php echo $this->createUrl('confirm',array('id'=>$val['refundId']));?>">财务确认</a><br>
	 <?php }else if( $val['state'] == '0') { ?>
		<a href="<?php echo $this->createUrl('check',array('id'=>$val['refundId']));?>">审核</a><br>
	   <?php }else {?>
		<a href="<?php echo $this->createUrl('view',array('id'=>$val['refundId']));?>">查看退货单</a><br>
		<a href="<?php echo $this->createUrl('print',array('id'=>$val['refundId']));?>" class="print">打印退货单</a><br>
		<?php }?>
	</td>
	<?php } ?>
    </tr>
