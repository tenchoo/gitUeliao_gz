  ...
  <colgroup><col><col width="15%"><col width="15%"><col width="15%"><col width="15%"></colgroup>
	...
	<tr>
	  ...
	  <td width="15%"><?php printf("%s %s", Order::quantityFormat($detail->num), $unitName);?></td>
		<?php if(!$index){?>
	  <td width="15%" rowspan="<?php echo $findRows;?>"><a href="<?php echo $this->createUrl('view',array('id'=>$item->warrantId))?>">查看</a> &nbsp; 
	  <?php if($item->state == tbWarehouseWarrant::STATE_APPLY){?>
	  撤消申请中
	  <?php }
	  else { ?>
	  <a href="<?php echo $this->createUrl('repeal',array('id'=>$item->warrantId))?>">申请撤消</a>
	  <?php } ?>
	  </td>
		<?php } ?>
    </tr>
