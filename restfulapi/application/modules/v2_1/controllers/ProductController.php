...
class ProductController extends Controller {
	...
	*/
	 ...
	 * 列表产品声音描述文件列表
	 */
	private function product_sound($product) {
		$dataList = array();
		$sounds = tbProductSound::model()->findAllByProductId($product->productId);
		if($sounds) {
			$dataList = array_map(function($row){
				$row->sound = Yii::app()->params['domain_images'].$row->sound;
				return $row->getAttributes(['title','isMain','sound','updateTime']);
			}, $sounds);
		}
		$this->showJson(true, "successfully", $dataList);
	}

	private function product_video() {
		$dataList = array();
		$ProductID = Yii::app()->request->getQuery("id");
		$videos = tbProductVideo::model()->findAllByProductId($ProductID);
		if($videos) {
			$dataList = array_map(function($row){
				$row->video = Yii::app()->params['domain_images'].$row->video;
				return $row->getAttributes(['title','isMain','video','updateTime']);
			}, $videos);
		}
		$this->showJson(true, "successfully", $dataList);
	}

	private function product_relates() {
		$ProductID = Yii::app()->request->getQuery("id");
		$product = tbProduct::model()->findByPk($ProductID);
		if(!$product) {
			$this->showJson(true, "successfully", array());
		}

		$list = $product->getCraftList( true );
		$list = array_map(function($row){
			$row['mainPic'] = Yii::app()->params['domain_images'] . $row['mainPic'] . '_200';
			return $row;
		}, $list);
		$this->showJson(true, "successfully", $list);
	}
