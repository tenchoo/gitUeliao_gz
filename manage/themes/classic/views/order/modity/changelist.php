	...
	<td rowspan="<?php echo $count;?>" >
     ...
     <td rowspan="<?php echo $count;?>">
		<?php if( $val['state'] != '7' && $val['cState'] == '0' ) { ?>
			<a href="<?php echo $this->createUrl('checkchange',array('id'=>$val['orderId']));?>" class="text-link">审核</a><br/>
		<?php } ?>
		<?php echo CHtml::link('订单详情',array('cview','id'=>$val['orderId'] )  );?>
	 </td>
	 <?php } ?>
    </tr>
	  <?php } ?>
