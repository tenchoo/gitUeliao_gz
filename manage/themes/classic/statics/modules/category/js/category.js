define(function(require, exports, module) {
  var categorys = require('api/ajax?action=category&categoryId=0&callback=define')['data']['childs'];
  var template = require('libs/arttemplate/3.0.0/template.js');
  var caches = {};

  function render(data) {
    return template.compile('{{each list}}<option value="{{$value.categoryId}}">{{$value.title}}</option>{{/each}}')({
      list: data
    });
  }
